...
to create-event
  ...
  if event-center != nobody [
    let event-size min-event-size + random max-event-size  ; random size between min-even-size and max-event-size
    let event-duration min-event-duration + random max-event-duration  ; andom duration
    ask event-center [
